...
Un factor clave que favoreceu o crecemento de Node.js foi o momento. Só uns anos antes, grazas ás aplicacións “Web 2.0” (como Flickr, Gmail, etc.) que mostraron ao mundo como podía ser unha experiencia moderna na web; JavaScript comezou a ser considerado como unha linguaxe *seria* para a programación.
...
Aprender unha linguaxe de programación en profundidade é difícil, así que ao usar a mesma linguaxe para realizar todo o traballo web, tanto no lado cliente como no lado servidor, é sen dubida unha vantaxe única.
...
A forma habitual de executar un programa Node.js é executar o comando `node` dispoñible globalmente (unha vez que instalas Node.js) e pasarlle o nome do arquivo que queres executar.
...
Agora, eses 2 arquivos dinnos que instalamos a versión `1.3.1`de cowsay, e a nosa [regra de versión npm](https://docs.npmjs.com/about-semantic-versioning) para as actualizacións é `^1.3.1`. Isto significa que npm pode actualizarse a parches e versións menores: `1.3.2`, `1.4.0`etc.
...
Cando entras en produción, se escribes `npm install`e o cartafol contén un arquivo  `package.json`, instálanse, xa que npm asume que se trata dunha implementación de desenvolvemento.
...
`setTimeout` devolve o ID do temporizador. Isto xeralmente non se usa, pero podes almacenar este ID e borralo se quere eliminar esta execución de función programada:
...
*Algúns navegadores (IE e Edge) implementan un método `setImmediate()` que fai exactamente esta mesma funcionalidade, pero non é estándar e [non está dispoñible noutros navegadores](https://caniuse.com/%23feat%3Dsetimmediate). Pero é unha función estándar en Node.js.*
...
Para evitar isto, podes programar un setTimeout recursivo para que se chame cando remate a función de devolución de chamada:
...
`setTimeout`e `setInterval`están dispoñibles en Node.js, a través do [módulo Temporizadores](https://nodejs.org/api/timers.html) .
...
No lado do backend, Node.js ofrécenos a opción de construír un sistema similar usando o [`events`módulo](https://nodejs.org/api/events.html) .
...
Podes ler todos os seus detalles na páxina do módulo de eventos en [https://nodejs.org/api/events.html](https://nodejs.org/api/events.html)

## 33. Traballar con descritores de arquivos en Node
...
Esa bandeira significa que abrimos o arquivo para ler.

Outras bandeiras que usarás habitualmente son:

- `r+`abra o arquivo para ler e escribir, se o arquivo non existe non se creará.
...
Node.js tamén ofrece un método de sincronización, que bloquea o fío ata que as estatísticas do arquivo estean listas:
...
A información do arquivo inclúese na variable stats. Que tipo de información podemos extraer usando as estatísticas?
...
## 35. Rutas de arquivos de nodos

Cada arquivo do sistema ten un camiño.
...
Inclúe este módulo nos seus arquivos usando
...
- `dirname`: obtén o cartafol principal dun arquivo
- `basename`: obtén a parte do nome do arquivo
- `extname`: obtén a extensión do arquivo
...
Podes obter o nome do arquivo sen a extensión especificando un segundo argumento para `basename`:
...
Alternativamente, podes usar a versión síncrona `fs.readFileSync()`:
...
Neste caso, unha mellor opción é ler o contido do arquivo mediante fluxos.
...
Alternativamente, podes usar a versión síncrona `fs.writeFileSync()`:
...
Por defecto, esta API substituirá **o contido do arquivo** se xa existe.
...
Todos eses métodos escriben o contido completo no arquivo antes de devolver o control ao seu programa (na versión asíncrona, isto significa executar a devolución de chamada)

Neste caso, unha mellor opción é escribir o contido do arquivo mediante fluxos.

## 38. Traballar con cartafoles

O módulo principal `fs` de Node.js ofrece moitos métodos útiles que podes usar para traballar con cartafoles.

### 38.1. Comproba se existe un cartafol

Usa `fs.access()`(e a súa contraparte baseada en promesas `fsPromises.access()`) para comprobar se o cartafol existe e Node.js pode acceder a el cos teus permisos.

### 38.2. Crear un novo cartafol

Usa `fs.mkdir()` ou `fs.mkdirSync()` ou `fsPromises.mkdir()` para crear un novo cartafol.
...
### 38.3. Ler o contido dun directorio

Usa `fs.readdir()` ou `fs.readdirSync()` ou `fsPromises.readdir()` para ler o contido dun directorio.

Este anaco de código le o contido dun cartafol, tanto arquivos como subcartafoles, e devolve o seu camiño relativo:
...
Tamén podes filtrar os resultados para devolver só os arquivos e excluír os cartafoles:
...
Usa `fs.rename()` ou `fs.renameSync()` ou `fsPromises.rename()` para renomear o cartafol. O primeiro parámetro é a ruta actual, o segundo a nova ruta:
...
Eliminar un cartafol que teña contido pode ser máis complicado do que precisas. Podes pasar a opción `{ recursive: true }` de eliminar recursivamente o contido.
...
:pencil: En Node , `v16.x` a opción `recursive` está **obsoleta** para `fs.rmdir` a API de devolución de chamada, en lugar de utilizar `fs.rm` para eliminar cartafoles que teñan contido neles:
...
Ou podes instalar e facer uso do módulo [`fs-extra`](https://www.npmjs.com/package/fs-extra), que é moi popular e ben mantido. É unha substitución do módulo `fs`, e que ofrece máis funcións enriba del.

Neste caso, se o que queres é o método `remove()`, podes instalalo usando
...
O módulo `fs` ofrece unha gran cantidade de funcionalidades moi útiles para acceder e interactuar co sistema de arquivos.
...
- `fs.access()`: comproba se o arquivo existe e Node.js pode acceder a el cos teus permisos.
- `fs.appendFile()`: engadir datos a un arquivo. Se o arquivo non existe, créase.
- `fs.chmod()`: cambia os permisos dun arquivo especificado polo nome de arquivo dado. Relacionados: `fs.lchmod()`,`fs.fchmod()`
- `fs.chown()`: cambia o propietario e o grupo dun arquivo especificado polo nome de arquivo dado. Relacionados: `fs.fchown()`,`fs.lchown()`
- `fs.close()`: pecha un descritor de arquivo
- `fs.copyFile()`: copia un arquivo
- `fs.createReadStream()`: crea un fluxo de arquivos lexible
- `fs.createWriteStream()`: crea un fluxo de arquivos escribible
- `fs.link()`: crea unha nova ligazón dura a un arquivo
- `fs.mkdir()`: crea un novo cartafol
- `fs.mkdtemp()`: crea un directorio temporal
- `fs.open()`: abre o arquivo e devolve un descritor de arquivo para permitir a manipulación do arquivo
- `fs.readdir()`: le o contido dun directorio
- `fs.readFile()`: ler o contido dun arquivo. Relacionados:`fs.read()`
- `fs.readlink()`: ler o valor dunha ligazón simbólica
- `fs.realpath()`: resolve os punteiros relativos á ruta do arquivo ( `.`, `..`) á ruta completa
- `fs.rename()`: renomear un arquivo ou cartafol
- `fs.rmdir()`: eliminar un cartafol
- `fs.stat()`: devolve o estado do arquivo identificado polo nome de arquivo pasado. Relacionados: `fs.fstat()`,`fs.lstat()`
- `fs.symlink()`: crea unha nova ligazón simbólica a un arquivo
- `fs.truncate()`: trunca ata a lonxitude especificada o arquivo identificado polo nome de arquivo pasado. Relacionados:`fs.ftruncate()`
- `fs.unlink()`: elimina un arquivo ou unha ligazón simbólica
- `fs.unwatchFile()`: deixa de ver os cambios nun arquivo
- `fs.utimes()`: cambia a marca de tempo do arquivo identificado polo nome de arquivo pasado. Relacionados:`fs.futimes()`
- `fs.watchFile()`: comeza a ver os cambios nun arquivo. Relacionados:`fs.watch()`
- `fs.writeFile()`: escribir datos nun arquivo. Relacionados:`fs.write()`

Unha cousa peculiar do módulo `fs` é que todos os métodos son asíncronos por defecto, pero tamén poden funcionar de forma sincrónica ao engadir `Sync`.
...
Isto supón unha gran diferenza no fluxo de aplicacións.

*Node.js 10 inclúe [soporte experimental](https://nodejs.org/api/fs.html%23fs_fs_promises_api) para unha API baseada en promesas.*

Por exemplo, imos examinar o método `fs.rename()`. A API asíncrona úsase cunha devolución de chamada:
...
A diferenza fundamental aquí é que a execución do teu script bloquearase no segundo exemplo, ata que a operación do arquivo teña éxito.

Podes usar a API baseada en promesas proporcionada polo módulo `fs/promises` para evitar usar a API baseada en devolución de chamada, o que pode provocar [un inferno de devolución de chamada](http://callbackhell.com/). Aquí tes un exemplo:
...
A API baseada na devolución de chamada pode aumentar o inferno de devolución de chamada cando hai demasiadas devolucións de chamada aniñadas. Daquela podemos usar a API baseada en promesas para evitalo:
...
O módulo `path` ofrece unha gran cantidade de funcionalidades moi útiles para acceder e interactuar co sistema de arquivos.
...
- `base`: o nome do arquivo + extensión
...
Ao especificar un segundo parámetro, utilizarase o primeiro `resolve` como base para o segundo:
...
Este módulo ofrece moitas funcións que podes usar para recuperar información do sistema operativo subxacente e do ordenador no que se executa o programa e interactuar con el.

```js
const os = require('os')
```

Hai algunhas propiedades útiles que nos indican algunhas cousas clave relacionadas co manexo de arquivos:

`os.EOL` dá a secuencia delimitadora de liñas. Isto é `\n` en Linux e macOS, e `\r\n` en Windows.
...
Podes atopalos todos en [https://nodejs.org/api/os.html#os_signal_constants](https://nodejs.org/api/os.html%23os_signal_constants) .
...
Devolve os detalles das interfaces de rede dispoñibles no teu sistema.
...
O módulo `events` ofrécenos a clase EventEmitter, que é clave para traballar con eventos en Node.js.
...
Devolve unha matriz de cadeas que representan os eventos rexistrados no obxecto `EventEmitter` actual:
...
Obtén a cantidade máxima de oíntes que se poden engadir a un obxecto `EventEmitter`, que por defecto é 10, pero que se poden aumentar ou reducir usando `setMaxListeners()`
...
Alias para engadir `emitter.removeListener()` en Node.js 10
...
Engade unha función de devolución de chamada que se chama cando se emite un evento por primeira vez despois de rexistrar isto. Esta devolución de chamada só se dará unha vez, nunca máis.
...
Cando engades un oínte usando `on` ou `addListener`, engádese o último na cola de oíntes e chámase o último. Usando `prependListener` engádese e chámase antes que outros oíntes.

### 42.11.`emitter.prependOnceListener()`

Cando engades un oínte usando `once`, engádese o último na cola de oíntes e chámase o último. Usando `prependOnceListener` engádese e chámase antes que outros oíntes.

### 42.12.`emitter.removeAllListeners()`

Elimina todos os oíntes dun obxecto `EventEmitter` que escoitan un evento específico:
...
Establece a cantidade máxima de oíntes que se poden engadir a un obxecto `EventEmitter`, que por defecto é 10 pero que se pode aumentar ou baixar.
...
Apunta á instancia global do obxecto Axente, que é unha instancia da clase `http.Agent`.
...
Máis adiante na descrición da clase `http.Agent`.

### 43.2. Métodos

#### 43.2.1.`http.createServer()`

Devolve unha nova instancia da clase `http.Server`.
...
Fai unha solicitude HTTP a un servidor, creando unha instancia da clase `http.ClientRequest`.

#### 43.2.3.`http.get()`

Similar a `http.request()`, pero configura automaticamente o método HTTP en GET e chama automaticamente `req.end()`.
...
Node.js crea unha instancia global da clase `http.Agent` para xestionar a persistencia e a reutilización das conexións para os clientes HTTP, un compoñente clave da rede HTTP de Node.js.
...
Un obxecto `http.ClientRequest` créase cando se chama `http.request()` ou `http.get()`.

Cando se recibe unha resposta, chámase o evento coa resposta `response`, cunha instancia `http.IncomingMessage` como argumento.

Os datos devoltos dunha resposta pódense ler de dúas formas:

- podes chamar ao método `response.read()`
- no manexador de eventos `response` podes configurar un detector de eventos para o evento `data`, para que poida escoitar os datos transmitidos.
...
Creado por un `http.Server` e pasado como segundo parámetro ao evento `request` que se desencadea.
...
O método que sempre chamarás no manexador é `end()`, que pecha a resposta, a mensaxe está completa e o servidor pode enviala ao cliente. Debe chamarse en cada resposta.
...
Despois de procesar as cabeceiras, podes envialas ao cliente chamando a `response.writeHead()`, que acepta o statusCode como primeiro parámetro, a mensaxe de estado opcional e o obxecto de cabeceiras.
...
Un obxecto `http.IncomingMessage` é creado por:

- `http.Server` ao escoitar o evento `request`
- `http.ClientRequest` ao escoitar o evento `response`

Pódese usar para acceder á resposta:

- estado usando os seus métodos `statusCode` e `statusMessage`
- cabeceiras usando o seu método `headers` ou`rawHeaders`
- Método HTTP usando o seu método `method`
- Versión HTTP usando o método `httpVersion`
- URL usando o método `url`
- socket subxacente usando o método `socket`
...
Son unha forma de manexar arquivos de lectura/escritura, comunicacións de rede ou calquera tipo de intercambio de información de extremo a extremo dunha forma eficiente.
...
Por exemplo, de xeito tradicional, cando lle dis ao programa que lea un arquivo, o arquivo é lido na memoria, de principio a fin, e logo procesado.

Usando fluxos les peza por peza, procesando o seu contido sen gardar todo na memoria.

O [módulo Node.js `stream`]( https://nodejs.org/api/stream.html) proporciona a base sobre a que se constrúen todas as API de transmisión. Todos os fluxos son instancias de [EventEmitter](https://nodejs.org/api/events.html%23events_class_eventemitter)
...
Un exemplo típico é ler arquivos desde un disco.

Usando o módulo `fs` Node.js, podes ler un arquivo e envialo a través de HTTP cando se establece unha nova conexión co teu servidor HTTP:
...
`readFile()` le o contido completo do arquivo e invoca a función de devolución de chamada cando remata.

`res.end(data)` na devolución de chamada devolverá o contido do arquivo ao cliente HTTP.

Se o arquivo é grande, a operación levará bastante tempo. Aquí está o mesmo escrito usando fluxos:
...
En lugar de esperar ata que o arquivo estea completamente lido, comezamos a transmitilo ao cliente HTTP en canto teñamos un anaco de datos listo para enviar.

### 44.4. tubo ()

O exemplo anterior usa a liña `stream.pipe(res)`: o método `pipe()` chámase no fluxo de arquivos.

Que fai este código? Leva a fonte e a encamíña a un destino.

Chámao no fluxo de orixe, polo que, neste caso, o fluxo de arquivo envíase á resposta HTTP.

O valor de retorno do método `pipe()` é o fluxo de destino, que é unha cousa moi conveniente que nos permite encadear varias chamadas `pipe()`, como esta:
...
- `fs.createReadStream()` crea un fluxo lexible nun arquivo
- `fs.createWriteStream()` crea un fluxo escribible nun arquivo
...
Obtemos o fluxo Readable do [módulo `stream`](https://nodejs.org/api/stream.html) e inicialímolo e implementamos o método `readable._read()`.

Primeiro crea un obxecto de fluxo:
...
Para crear un fluxo de escritura ampliamos o obxecto base `Writable` e implementamos o seu método `_write()`.
...
Usando o método `write()` de fluxo:
...
No exemplo anterior, chámase `end()` dentro dun oínte ao evento `close` no fluxo lexible para garantir que non se chame antes de que todos os eventos de escritura pasaran pola canalización, xa que facelo faría que se emitise un evento `error`. Ao chamar `destroy()` ao fluxo lexible fai que `close` se emita o evento. O oínte do evento `close` no fluxo escribible demostra a finalización do proceso xa que se emite despois da chamada a `end()`.

### 44.12. Como crear un fluxo de transformación

Obtemos o fluxo Transform do [módulo `stream`](https://nodejs.org/api/stream.html) e inicialímolo e implementamos o método `transform._transform()`.

Primeiro crea un obxecto de fluxo de transformación:
...
Node.js asume que sempre se está a executar nun ambiente de desenvolvemento. Podes indicar a Node.js que estás a executar en produción configurando a variable de ambiente `NODE_ENV=production`.
...
no intérprete de comandos, pero é mellor poñelo no arquivo de configuración do intérprete de comandos (por exemplo, `.bash_profile` co intérprete de comandos de Bash) porque, se non, a configuración non persiste en caso de reiniciar o sistema.

Tamén pode aplicar a variable de ambiente antecedendoa ao comando de inicialización da túa aplicación:
...
Configurar o ambiente xeral en `production` garante que
...
Créase unha excepción usando a palabra clave `throw`:
...
Un obxecto de erro é un obxecto que é unha instancia do obxecto Error ou que amplía a clase Error, que se proporciona no [módulo principal de Erro](https://nodejs.org/api/errors.html) :
...
Un manexador de excepcións é unha instrución `try`/`catch` .

Calquera excepción suscitada nas liñas de código incluídas no bloque `try` tratarase no bloque `catch` correspondente:
...
neste exemplo está o valor `e` de excepción.
...
Para resolver isto, escoita o evento `uncaughtException` no obxecto `process`:
...
Non é necesario importar o módulo principal `process` para iso, xa que se inxecta automaticamente.
...
Como saber onde ocorreu o erro? Realmente non o sabes, pero podes xestionar os erros en cada unha das funcións que chamas ( `doSomethingX`), e dentro do controlador de erros lanzar un novo erro, que vai chamar ao controlador externo `catch`:
...
Para poder xestionar erros localmente sen manexalos na función que chamamos, podemos romper a cadea. Podes crear unha función en cada unha e procesar a excepción `then()`:
...
Usando async/wait, aínda necesitas detectar erros e podes facelo deste xeito:
...
Aquí tes unha mostra Hello World do servidor web HTTP:
...
Imos analizalo brevemente. Incluímos o [módulo `http`](https://nodejs.org/api/http.html) .

Usamos o módulo para crear un servidor HTTP.

O servidor está configurado para escoitar no porto especificado, `3000`. Cando o servidor está listo, chámase a función de devolución de chamada `listen`.

A función de devolución de chamada que pasamos é a que se vai executar en cada solicitude que se reciba. Sempre que se recibe unha nova  solicitude, chámase o [evento `request`](https://nodejs.org/api/http.html%23http_event_request), proporcionando dous obxectos: unha solicitude[`http.IncomingMessage`](https://nodejs.org/api/http.html%23http_class_http_incomingmessage) (un obxecto)  e unha resposta [`http.ServerResponse`](https://nodejs.org/api/http.html%23http_class_http_serverresponse)(un obxecto).
...
A forma máis sinxela de realizar unha solicitude HTTP usando Node.js é usar a [biblioteca Axios](https://github.com/axios/axios) :
...
Do mesmo xeito que facer unha solicitude HTTP GET, pode utilizar a [biblioteca Axios](https://github.com/axios/axios) para realizar unha solicitude POST:
